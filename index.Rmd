...
#Not working:Qdata=select(Qdata,('Cross.Section.Name'='Cross-Section Name'),('Discharge'='Discharge/tcfs'), 'Year', 'Month','Day',('Time'='Start Time'))
head(Qdata)
#Not working: Qdata$Date[is.na(Qdata$Date)]=as.Date( with(Qdata,paste(Year, Month , Day , sep = "-" ),"12-31-9999", format = "%Y-%m-%d" ))
...
#Qdata$Date[is.na(Qdata$Date)]=as.Date( with(Qdata,paste(Year, Month , Day , sep = "-" ),"12-31-9999", format = "%Y-%m-%d" ))
head(Qdata)
```
...
#setcolorder(Qdata,c("Date", "Year", "Month", "Day", "Cross.Section.Name", "Discharge"))

#QDT<-as.data.table(Qdata)%>%
# select(c("Date"="index","Discharge"))
#head(QDT)
#Qxts<-as.xts.data.table(QDT)
#head(Qxts)
#xts(QDT[, -1], order.by=as.POSIXct.Date(QDT$index))

```
```{r}
#MQDT<-data.table(meanQdata)
#as.xts.data.table(MQDT)
#xts(MQDT[, -1], order.by=as.Date(meanQdata$Date))
#head(MQDT)
```
# Results
...
```{r}
  ...
  m=plot(x,y,
       ...
       ylab="Discharge (cfs)",
       xlab="Year",
       main="Annual Mean Discharge of the Niagara River",
       col="blue"
       
  )
  
```
The discharge measurements have been averaged for each year and plotted over time. 
